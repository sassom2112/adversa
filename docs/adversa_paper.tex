...
%% ── Front matter ────────────────────────────────────────────────────────────
\title{find-evil: Autonomous Adversarial Red–Blue Learning for\\
...
\begin{abstract}
find-evil automates both sides of the adversarial arms race in digital
forensics and incident response (DFIR). A Red Agent draws from
\num{49519} real Windows Sysmon events sourced from the OTRF Mordor
Security Datasets and evolves evasion variants when caught. A Blue
Agent detects, misses, and learns—without human labeling or manually
authored rules. Over \num{4500} iterations, the system recovered from
a domain-gap collapse at iteration~10 (10\% detection) to 76\%
detection with F1~=~0.72, generating \num{2031} adversarial evasion
variants and extracting 151 grounded signals across 8 MITRE ATT\&CK
techniques. The trained signals are exported as 11 operational rules
that drive a two-pass forensic investigation system: a generic
deterministic triage stage followed by a fully agentic Claude-powered
investigation loop, validated on four machines from an APT intrusion
with Chinese-language artifacts. A Forensic Auditor challenges every
detected technique under a bounded tool-call budget, correctly
refuting false positives on two machines. Custom agent findings trace
to specific forensic tool calls in an append-only audit log.
\end{abstract}

\keywords{digital forensics, adversarial machine learning, MITRE
ATT\&CK, DFIR automation, Sysmon, threat detection, adversarial signal learning}
