...
physical artifact verification before confirming any finding. The system was validated on three hosts from an APT intrusion.
On nfury, triage scored 100/100 across 19 detected techniques;
the Auditor confirmed 15 (including T1003.002 SAM credential dump,
T1055 process injection, T1569.002 PsExec lateral movement, and
...
reuse across hosts. On nromanoff, a distinct tool family
(\texttt{spinlock.exe}) connected to an external C2
(\texttt{199.73.28.114:443}); 3 of 7 detected techniques were
confirmed, 4 refuted---the identical discrimination pattern seen
across all three hosts. In total, 31 of 43 detected techniques
were confirmed and 12 correctly refuted. All findings trace to
specific forensic tool calls in an append-only audit log; every
confirmed technique can be independently reproduced with one shell
...
\Cref{tab:results} summarises findings from three hosts investigated
...
nromanoff was run standalone; it carries a distinct tool family
(\texttt{spinlock.exe}) and a real external C2 not shared with nfury or tdungan.
...
nromanoff (10.3.58.5)  & 100 & 100 &  3 (see \cref{tab:nromanoff}) &  4 \\
\midrule
\textbf{Total}         &     &     & \textbf{31 of 43}             & \textbf{12} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{nromanoff.}
Investigated standalone (no prior-campaign IOCs injected).
Combined disk and memory triage scored 100/100 across 7 detected
techniques. The Auditor challenged all 7 and returned
\textbf{3 confirmed, 4 refuted.} \Cref{tab:nromanoff} lists all confirmed
techniques.

\begin{table}[ht]
\centering\small
\caption{nromanoff confirmed techniques. Source indicates whether the
  confirming artifact was found on disk, in memory, or both.}
\label{tab:nromanoff}
\begin{tabular}{@{}lll@{}}
\toprule
...
\midrule
T1003.002 & SAM Credential Dump         & disk+memory \\
T1055     & Process Injection           & memory \\
T1569.002 & PsExec (System Services)    & disk \\
\bottomrule
\end{tabular}
\end{table}

Key physical artifacts: \texttt{spinlock.exe}---a PyInstaller-bundled
Python~2.5 backdoor (\texttt{python25.dll}, temp dir
\texttt{\_MEI29562\textbackslash}), a distinct tool family from the
httppump variants on nfury and tdungan. The execution chain was
recovered from memory: \texttt{a.exe} (PID~3264, dropped to
\texttt{vibranium\textbackslash AppData\textbackslash Local\textbackslash Temp})
$\to$ \texttt{cmd.exe} $\to$ \texttt{spinlock.exe}, subsequently
re-launched via \texttt{PSEXESVC.EXE} into SYSTEM context (T1569.002).
\texttt{spinlock.exe} established a TLS connection to an external C2
at \texttt{199.73.28.114:443} presenting a self-signed certificate with
\texttt{CN=199.73.28.114}---the only confirmed external C2 node in the
campaign. A rogue \texttt{svchost.exe} at
\texttt{system32\textbackslash dllhost\textbackslash svchost.exe}
(the same masquerade path as tdungan) bound to
\texttt{127.0.0.1:5678} was also active in memory.
\texttt{windows.hashdump} returned
\texttt{SRL-Helpdesk: 4c3f5e9fe4c8fc2f99d47cbb25d7d193}---the
identical hash seen on nfury and tdungan, confirming the attacker
planted and reused this service account across all three hosts.
T1055 was confirmed via an anonymous \texttt{PAGE\_EXECUTE\_READWRITE}
VadS region in \texttt{LogonUI.exe}.

Refuted (4): T1071.001, T1134, T1547.001, T1574---all memory-only
signals with no disk corroboration, the identical refutation pattern
seen on both nfury and tdungan.

Total runtime: 692\,s ($\approx$11.5\,min); total cost: $\approx$\$10.

\subsection{Cross-Host Campaign Correlation}
\label{sec:campaign}

Three hosts, consistent attacker infrastructure, and a single confirmed
initial access vector. The correlations are grounded in physical
artifacts on each image---not in shared model state or triage scoring.
...
\textbf{SRL-Helpdesk NTLM hash identical on all three hosts.}
Volatility~3 \texttt{windows.hashdump} returned
\texttt{SRL-Helpdesk: 4c3f5e9fe4c8fc2f99d47cbb25d7d193}
on nfury, tdungan, and nromanoff. An identical NTLM hash for an
attacker-created service account across three separate domain hosts
is conclusive evidence of a single operator deploying a shared
credential pool across the campaign.

\textbf{IOC propagation confirmed across all three hosts.}
The \texttt{a.exe} filename IOC extracted from nfury was recognised
as a priority signal during the tdungan investigation;
\texttt{A.EXE-239305EA.pf} and \texttt{A.EXE-0F3A0E12.pf} prefetch
entries confirmed staged payload delivery on tdungan.
\texttt{a.exe} also appears in nromanoff's IOC set---the same dropper
filename, different binaries, three hosts. This is the campaign mode
delivering its intended output: a confirmed IOC on one host shortens
the tool-call budget required to verify the same tooling on subsequent
hosts.
...
\textbf{nromanoff: distinct tool family, external C2.}
nromanoff (10.3.58.5) carries \texttt{spinlock.exe}---a PyInstaller
Python backdoor with no SHA-256 overlap with the httppump variants on
nfury or tdungan---and connects to a real external IP
(\texttt{199.73.28.114:443}) rather than the internal relay
\texttt{192.168.1.5}. This suggests the operator deployed different
staging infrastructure per host. The \texttt{vibranium} account and
PsExec confirmed on nromanoff are consistent with the lateral
movement profile, but the tool family is distinct. nromanoff is
the only host where an external C2 is confirmed in a TLS session with
a self-signed certificate---physical evidence of live C2 communication
at the time of acquisition.

\textbf{Consistent Auditor discrimination across all three hosts.}
Every host returned exactly 4 refuted findings, all memory-only signals
without disk corroboration. This is not tuned behaviour: the refutation
criterion (positive tool return on a real forensic binary) is applied
uniformly. Three independent investigations, three identical refutation
patterns---the strongest evidence the discrimination is structural,
not coincidental.
...
\textbf{LLM investigation is non-deterministic.} A re-run of nfury on
the same evidence with the same code produced 8 detected techniques and
6 confirmed---versus 19 detected and 15 confirmed on the original run.
Same disk image, same pipeline, different LLM exploration path.
The disk investigation agent (Pass~2, 75-call budget) surfaced
13 disk-resident techniques on the first run and 3 on the second;
it took different branching decisions each time.
The confirmed findings from either run are individually valid---every
CONFIRMED finding in both transcripts cites a real tool call and a
real artifact on disk. But the \emph{set} of confirmed findings is
not stable across runs. This is a fundamental property of agentic
LLM investigation, not a bug. Mitigation: run multiple passes and
take the union of confirmed findings; or accept the first run's output
as a lower bound with the audit log as the reproducibility guarantee.
...
exist in any manually authored rule set. Three hosts from an APT intrusion were investigated autonomously. On
...
On nromanoff, a distinct Python backdoor (\texttt{spinlock.exe})
with a real external C2 (\texttt{199.73.28.114:443}) was confirmed;
3 of 7 detections confirmed, 4 refuted---the same discrimination
pattern as the other two hosts.
Thirty-one techniques confirmed across 43 detected, 12 correctly
refuted by the Auditor---all within $\approx$45~minutes total,
at under \$45 combined.
