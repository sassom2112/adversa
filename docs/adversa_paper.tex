...
was validated on the nfury host from an APT intrusion: triage scored
100/100 across 9 MITRE techniques; the Auditor confirmed two
(T1003.002 credential dumping via SAM hive extraction; T1055 process
injection via memory analysis), refuted seven, and produced a complete
IOC set linking the \texttt{vibranium} domain account, httppump C2,
and \texttt{a.exe} loader. All findings
...
\subsection{Results on nfury (Current Pipeline)}

\Cref{tab:results} summarises findings from the nfury investigation
using the full current pipeline (corpus-calibrated weights, decoupled
passes, fixed auditor). Additional hosts are pending re-run with the
current pipeline.

\begin{table}[!ht]
\centering
\caption{Pipeline results on nfury (current pipeline).
  Triage score reflects Pass~1+2 weighted signal matches; adjusted
  score reflects the Forensic Auditor's confirmed-technique sum after
...
\toprule
Machine (IP)      & Triage & Adjusted & Confirmed & Refuted \\
\midrule
nfury (10.3.58.6) & 100 & 70 & T1003.002, T1055 & 7 techniques \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{nfury.}
Triage scored 100/100 across 9 detected techniques. The Auditor
challenged all 9 in parallel over 25 argumentation rounds and confirmed
two on physical artifacts. T1003.002 (SAM credential dump) was confirmed
via registry hive extraction—the SAM hive at
\texttt{Windows/System32/config/SAM} was verified accessible and the
attacker account \texttt{vibranium} was confirmed via ProfileList
analysis (SID \texttt{S-1-5-21-2036804247-3058324640-2116585241-1673}).
T1055 (Process Injection) was confirmed via Volatility~3 memory
analysis of the \texttt{a.exe} loader. Seven techniques were refuted.
The attack chain includes httppump C2 at \texttt{199.73.28.114/ads/}
and exfiltration via \texttt{system4.rar}. Total runtime: 17 minutes;
total cost: \$14.
...
deployment: on nfury, the Auditor correctly refuted 7 of 9 triage
detections, reducing the analyst workload from nine technique
investigations to two.
...
exist in any manually authored rule set. The nfury host from an APT intrusion with Chinese-language artifacts
was investigated autonomously; the Forensic Auditor correctly refuted
7 of 9 triage detections and confirmed T1003.002 and T1055 on physical
artifacts. The \texttt{vibranium} domain account (SID confirmed in
ProfileList) and httppump C2 infrastructure were fully characterized
from a single autonomous run.
