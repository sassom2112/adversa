...
Password hashes are stored in /var/lib/adversa/passwords/{examiner}.json
(0o600, directory 0o700).
...
_LOCKOUT_FILE = Path.home() / ".adversa" / ".password_lockout"
_MIN_PASSWORD_LENGTH = 8
_PASSWORDS_DIR = Path("/var/lib/adversa/passwords")
...
def require_confirmation(config_path: Path, analyst: str) -> tuple[str, str | None]:
    """Require password confirmation. Returns ('password', raw_password_string).

    Reads from /dev/tty — blocks AI-via-Bash and expect-style automation.
    Raises SystemExit on failure, lockout, or missing password.
    """
    if not has_password(config_path, analyst):
        print(
            "No approval password configured. Set one with:\n  adversa config --setup-password\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def setup_password(
    ...
    """Set up a new password. Returns raw password (needed for HMAC derivation)."""
    passwords_dir = passwords_dir or _PASSWORDS_DIR
...
def _check_lockout(analyst: str) -> None:
    if _recent_failure_count(analyst) >= _MAX_PASSWORD_ATTEMPTS:
        ...
        remaining = int(_LOCKOUT_SECONDS - (now - min(recent))) if recent else _LOCKOUT_SECONDS
        print(
            f"Password locked. Too many failed attempts. Try again in {max(remaining, 1)} seconds.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def getpass_prompt(prompt: str) -> str:
    ...
    This is intentional: approvals require an interactive terminal.
    An AI agent running via Bash cannot satisfy this requirement.
    """
    if not _HAS_TERMIOS:
        raise RuntimeError(
