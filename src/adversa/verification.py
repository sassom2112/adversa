"""HMAC verification ledger for approved findings.

The verification ledger lives at /var/lib/adversa/verification/{case-id}.jsonl.
This path is outside the user's home directory and unreachable by the Claude
Code sandbox from any CWD.

Each entry records an HMAC-SHA256 over the finding text, keyed by
PBKDF2(password, salt). The LLM cannot forge entries because it does not
know the password-derived key.
...
VERIFICATION_DIR = Path("/var/lib/adversa/verification")
...
def write_ledger_entry(case_id: str, entry: dict) -> None:
    """Append entry to /var/lib/adversa/verification/{case_id}.jsonl."""
...
def verify_items(case_id: str, password: str, salt: bytes, examiner: str) -> list[dict]:
    """Verify HMAC for all findings belonging to examiner."""
